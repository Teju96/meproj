...
Connection conn = null;
          ...
          s1.removeAllElements();
                                    ...
                                    ResultSet rs = st.executeQuery("select * from time ORDER BY flength ASC LIMIT 3");
                                    while(rs.next())
                                    {
                                        
                                       if(rs.getString("username").equals(userdetails1))
                                       {
                                           String size=rs.getString("flength");
                                           String cost=rs.getString("ctime");
...
System.out.println("sc1****"+sc1.size());
                        double[] cr1 = new double[s1.size()]; 
                        double[] cr2 = new double[s1.size()]; 
                        double[] cr3 = new double[s1.size()];
                       
                        String[] labels = new String[s1.size()];
...
try {
    ...
    for(int i=0;i<s1.size();i++) {
         ...
         cr2[1]=13;
         cr2[2]=16;
         cr3[i]=Double.parseDouble(s2.get(i).toString())/2;
         labels[i]=bb;
    }
    String param = "";
                      ...
                      // c.setPlotArea(50, 25,600,250, 0xffffc0, 0xffffe0);
                       ...
                       c.addLegend(55, 18, false, "",10).setBackground(Chart.Transparent);
                         BarLayer layer=c.addBarLayer(Chart.Side,3);
                          layer.addDataSet(cr2,0x80ff80,"straight forward approach");
                           layer.addDataSet(cr1,0xff8080,"primitive approach");
                       
                      
                       layer.addDataSet(cr3,0x8080ff,"delegation approach");
                        c.xAxis().setLabels(labels);
                        c.xAxis().setLabelStep(0);
                        c.xAxis().setLabelStep(0);
                        
                        c.addTitle("setup time cost", "Times New Roman Bold Italic", 15).setBackground(0xccccff, 0x000000, Chart.glassEffect());
                        c.yAxis().setTitle("setup time cost", "Arial Bold Italic", 12);
                        c.xAxis().setTitle("Number of segments", "Arial Bold Italic", 12);
