...
Connection conn = null;
			...
			Statement st =conn.createStatement();
                        ...
                        //ResultSet rs = st.executeQuery("select * from recommend where uname='"+user+"'");
                                    ...
                                    ResultSet rs = st.executeQuery("select * from atime LIMIT 3");
                                    while(rs.next())
                                    {
                                       if(rs.getString("uname").equals(userdetails1))
                                       {
                                          ...
                                          s1.add("60");
                                           s1.add("80");
                                           s1.add("100");           
System.out.println("s1"+s2.size());
System.out.println("sc1"+sc2.size());
                     ...
                     c.setPlotArea(60, 60,500,200);
                      ...
                      // c.setPlotArea(50, 25,600,250, 0xffffc0, 0xffffe0);
                       ...
                       c.addLegend(55, 18, false, "",10).setBackground(Chart.Transparent);
                        ...
                        layer.addDataSet(cr1,0xff8080,"verification for repair");
                        layer.addDataSet(cr3,0x80ff80,"authenticator re-regeneration");
                      
                        layer.addDataSet(cr2,0x8080ff,"future enhancement authenticator regeneration");
                        c.xAxis().setLabels(labels);
                        c.xAxis().setLabelStep(0);
                        ///c.addTitle("time cost during repair", "Times New Roman Bold Italic", 15).setBackground(0xccccff, 0x000000, Chart.glassEffect());
                        c.yAxis().setTitle("time cost during repair", "Arial Bold Italic", 12);
                        c.xAxis().setTitle("Time for repair with different S", "Arial Bold Italic", 12);
